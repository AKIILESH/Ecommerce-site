...
function getCategoryFromURL() {
    ...
    return params.get("category"); // Gets the category from URL, e.g., 'tshirts'
}

// Function to filter and display products based on category
function displayProducts() {
    const category = getCategoryFromURL();  // Get category from URL
    const products = document.querySelectorAll(".product"); // Get all product elements
    products.forEach(product => {
        const productCategory = product.getAttribute("data-category"); // Get category from the data attribute
        if (category && productCategory !== category) {
            product.style.display = "none"; // Hide the product if the category does not match
        } else {
            product.style.display = "block"; // Display the product if it matches the category or no category is selected
        }
    });
}
